...
#' @export getChannels
#' @examples
#' msg_example <-getChannels(inpath=system.file("extdata/msg",package="MSGtools"),
...
getChannels <- function (inpath, type="rst",channels= c(
  ...
  result <-  raster::stack(paste0(inpath,"/",scenes[substr(scenes,20,28)%in%x]))
  # set non clouded areas to NA:
  if (!is.null(mask)){
    result <- raster::reclassify(result, cbind(mask,NA))
  }
